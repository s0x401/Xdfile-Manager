...
type ConfigType struct {
	Theme string `toml:"theme" comment:"Xdfile Manager theme selection\nchange your theme"`
	...
	CdOnQuit               bool   `toml:"cd_on_quit" comment:"\nCd on quit when leaving Xdfile Manager."`
	DefaultOpenFilePreview bool   `toml:"default_open_file_preview" comment:"\nWhether to open file preview automatically every time Xdfile Manager is opened."`
	...
	DefaultDirectory       string `toml:"default_directory" comment:"\nThe path of the first file panel when Xdfile Manager is opened."`
