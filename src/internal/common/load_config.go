...
import (
	...
	"github.com/s0x401/xdfile-manager/src/config/icon"
	...
	if err != nil {
		...
		if errors.As(err, &loadError) && loadError != nil {
			if loadError.MissingFields() && !variable.FixConfigFile {
				...
				userMsg += "\nTo add missing fields to the configuration file automatically run xdfile " +
					"with the --fix-config-file flag `xdfile --fix-config-file`"
			}
	...
	if err != nil {
		...
		if errors.As(err, &loadError) {
			if loadError.MissingFields() && !variable.FixHotkeys {
				// Had missing fields and we did not fix
				userMsg += "\nTo add missing fields to the hotkeys file automatically run xdfile " +
					"with the --fix-hotkeys flag `xdfile --fix-hotkeys`"
			}
			toExit = loadError.IsFatal()
		}
		if toExit {
...
// LoadAllDefaultConfig : Load all default configurations from embedded xdfile_config folder into global
