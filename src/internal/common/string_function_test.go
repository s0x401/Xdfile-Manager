...
func TestStringTruncate(t *testing.T) {
	var inputs = []struct {
		...
		{TruncateMiddleText, "TruncateMiddleText", "Hello", 5, "...", "Hello"},
		{TruncateMiddleText, "TruncateMiddleText", "Hello", 100, "...", "Hello"},
		{TruncateMiddleText, "TruncateMiddleText", "中文文件报告", 8, "...", "中...告"},
	}
...
func TestFormatFileSizeUsesSelectedUnitSystem(t *testing.T) {
	original := Config.FileSizeUseSI
	defer func() {
		Config.FileSizeUseSI = original
	}()

	Config.FileSizeUseSI = false
	if got := FormatFileSize(1536); got != "1.50 KiB" {
		t.Fatalf("expected binary size, got %q", got)
	}

	Config.FileSizeUseSI = true
	if got := FormatFileSize(1500); got != "1.50 kB" {
		t.Fatalf("expected SI size, got %q", got)
	}
}
